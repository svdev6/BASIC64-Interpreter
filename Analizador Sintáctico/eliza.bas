...
50 REM     EDITED AND MODIFIED FOR DARTMOUTH BASIC BY SANTIAGO VALENCIA DIAZ
60 REM     UNIVERSIDAD TECNOLOGICA DE PEREIRA, COMPILADORES 2024-1
70 REM     
...
111 LET N2 = 12
112 LET N3 = 112
120 FOR X = 1 TO N1 + N2 + N3
121   READ Z$
122 NEXT X
123 REM SAME AS RESTORE
130 FOR X = 1 TO N1
140   READ S(X), L
141   LET R(X) = S(X)
142   LET N(X) = S(X) + L - 1
...
370 LET K = 36
371 GOTO 570
372 REM     WE DIDN'T FIND ANY KEYWORDS
380 REM     
...
610 LET R(K) = R(K) + 1
611 IF R(K) <= N(K) THEN 620 
612   LET R(K) = S(K)
620 IF RIGHT$(F$, 1) = "*" THEN 630
621   PRINT F$
622   LET P$ = I$
623   GOTO 170
